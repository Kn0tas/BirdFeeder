...
static esp_err_t max17048_read_reg(uint8_t reg, uint8_t *data, size_t len) {
  ...
  return i2c_master_transmit_receive(s_max_handle, &reg, 1, data, len, -1);
}
...
static esp_err_t max17048_write_reg16(uint8_t reg, uint16_t value) {
  ...
  return i2c_master_transmit(s_max_handle, buf, sizeof(buf), -1);
}
...
esp_err_t max17048_init(void) {
  ...
  esp_err_t err =
      i2c_master_bus_add_device(bus_handle, &dev_cfg, &s_max_handle);
  if (err != ESP_OK) {
  ...
  uint8_t ver[2] = {0};
  err = max17048_read_reg(REG_VERSION, ver, sizeof(ver));
  if (err != ESP_OK) {
